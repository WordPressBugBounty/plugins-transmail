...
`}),o.innerHTML=r}else alert("Failed to filter data.")}).catch(e=>{})}}(e)})});else if(t){if(t.innerHTML="", $i=1,tempData.length<1){tempData.push({index:tempData.length+1,fromName:"",fromEmail:"",token:""});let a=document.createElement("div");a.classList.add("form-row-group-wrapper"),a.innerHTML=`
...
<div>
<input type="password" name="transmail_send_mail_token_1" value="" placeholder="Enter agent token" class="form--input" onchange="updateTempData(0, 'token', this.value)" />
</div>
</div>
<div class="form-row-group-default">Default</div>
`,t.appendChild(a)}else tempData.forEach((e,o)=>{let r=document.createElement("div");r.classList.add("form-row-group-wrapper");let a="",n="",l="";document.getElementById("row-"+e.id),void 0!=e.isConnected&&(e.isConnected?n=`
...
<div>
<input type="password" name="transmail_send_mail_token_${$i}" value="${e.token}" placeholder="Enter agent token" class="form--input" onchange="updateTempData(${$i-1}, 'token', this.value)" />
</div>
</div>
<p style="
margin-block: 2px 0;
...
<button onclick="addEntry()" title="Add new agent" id="toggleButtonPrev" type="button" class="add-button no-border">
Add new
</button>
</div>
...
`,t.appendChild(r)}}r(),o&&o.forEach(e=>{e.addEventListener("click",function(){let e=this.getAttribute("data-filter"),t=document.querySelectorAll(".zm-page-content-table tbody tr");t.forEach(t=>{let o=t.querySelector("td:nth-child(7)").textContent;"ALL"===e||o===e?t.style.display="":t.style.display="none"})})}),window.validateForm=function(){document.getElementById("tempDataCount").value=tempData.length,document.getElementById("tempData").value=tempData;let e=document.querySelectorAll("#form-container .form-row-group input"),t=/^[^\s@]+@[^\s@]+\.[^\s@]+$/;let o=!0;return e.forEach(e=>{let r=e.value.trim(),a=e.name;e.classList.remove("invalid-input"),""===r||a.includes("email")&&!t.test(r)&&(e.classList.add("invalid-input"),o=!1)}),"false"===o&&(alert("Please enter the valid data"),r()),o},window.updateTempData=function(e,t,o){if(e<0||e>=tempData.length){return}let r=tempData[e];r&&(r[t]=o,document.getElementById("tempDataCount").value=tempData.length,document.getElementById("tempData").value=tempData)},document.querySelectorAll("tbody .zm-page-content-table-row-checkbox input").forEach(e=>{e.addEventListener("change",a)}),window.deleteSelectedRows=function(){let e=Array.from(document.querySelectorAll(".row-checkbox:checked")).map(e=>e.getAttribute("data-id"));if(0===e.length){alert("No rows selected for deletion.");return}confirm("Are you sure you want to delete the selected records?")&&fetch(ajaxurl,{method:"POST",headers:{"Content-Type":"application/x-www-form-urlencoded; charset=UTF-8"},body:new URLSearchParams({action:"delete_selected_logs",ids:JSON.stringify(e),nonce:myAjax.nonce})}).then(e=>e.json()).then(t=>{t.success?e.forEach(e=>{document.getElementById("row-"+e).remove()}):alert("Failed to delete records.")})},window.deleteField=function(e){e>=0&&e<tempData.length&&tempData.splice(e,1),r()},window.makeDefault=function(e){tempData.forEach(function(e,t){e.isDefault=!1}),tempData[e].isDefault=!0,document.getElementById("defaultData").value=parseInt(e)+1,r()},window.addEntry=function(){tempData.length<3?(tempData.length&&tempData.map(e=>e.index),tempData.push({index:tempData.length+1,fromName:"",fromEmail:"",token:""})):alert("Agent limit exceeded"),r()},window.deleteDiv=function(e){var t=e.closest(".form-row-group");t&&t.remove()},e&&e.addEventListener("click",()=>{confirm("Are you sure you want to show this record?")&&fetch(`${ajaxurl}?action=get_mailagents_count`,{method:"GET",headers:{"Content-Type":"application/x-www-form-urlencoded; charset=UTF-8"}}).then(e=>e.json()).then(e=>{void 0!==e.length?addEntry():alert("Failed to fetch agents count")}).catch(e=>{})}),t&&t.addEventListener("click",()=>{confirm("Are you sure you want to show this record?")&&fetch(`${ajaxurl}?action=get_mailagents_count`,{method:"GET",headers:{"Content-Type":"application/x-www-form-urlencoded; charset=UTF-8"}}).then(e=>e.json()).then(e=>{void 0!==e.length?addEntry():alert("Failed to fetch agents count")}).catch(e=>{})}),window.deleteRow=function(e){var t=e;confirm("Are you sure you want to delete this record?")&&fetch(ajaxurl,{method:"POST",headers:{"Content-Type":"application/x-www-form-urlencoded; charset=UTF-8"},body:new URLSearchParams({action:"delete_failed_email",id:t,nonce:myAjax.nonce})}).then(e=>e.json()).then(e=>{e.success?(alert("Record deleted successfully!"),document.querySelector(`#row-${t}`).remove()):alert("Failed to delete record: "+(e.data||"Unknown error"))}).catch(e=>{alert("An error occurred while trying to delete the record.")})},document.querySelectorAll(".retry-button").forEach(function(e){e.addEventListener("click",function(e){e.preventDefault();var t=this.getAttribute("data-id");confirm("Are you sure you want to retry this record?")&&fetch(ajaxurl,{method:"POST",headers:{"Content-Type":"application/x-www-form-urlencoded; charset=UTF-8"},body:new URLSearchParams({action:"retry_failed_email",id:t,nonce:myAjax.nonce})}).then(e=>e.json()).then(e=>{e.success?(alert("Retry email succeeded."),deleteRow(t)):alert("Failed to retry email: "+(e.data||"Unknown error"))}).catch(e=>{alert("An error occurred while trying to retry the record.")})})}),window.deleteRow=function(e){var t=e;confirm("Are you sure you want to delete this record?")&&fetch(myAjax.ajaxurl,{method:"POST",headers:{"Content-Type":"application/x-www-form-urlencoded; charset=UTF-8"},body:new URLSearchParams({action:"delete_failed_email",id:t,nonce:myAjax.nonce})}).then(e=>e.json()).then(e=>{e.success?(alert("Record deleted successfully!"),document.querySelector(`#row-${t}`).remove()):alert("Failed to delete record: "+(e.data||"Unknown error"))}).catch(e=>{alert("An error occurred while trying to delete the record.")})},document.querySelectorAll(".delete-button").forEach(function(e){e.addEventListener("click",function(){deleteRow(this.getAttribute("data-id"))})})});
